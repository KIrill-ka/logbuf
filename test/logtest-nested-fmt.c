...
void sub1(logbuf_t *lb, uint8_t argn)
{
...
void sub3_1(logbuf_t *lb, uint8_t argn)
{
 if(lb != NULL) logbuf_fmtauto(lb, &argn, "sub%u()", 31);
}

void sub3(logbuf_t *lb, uint8_t argn)
{
 if(lb != NULL) {
...
int main()
{
 ...
 if(lb != NULL) {
  logbuf_fmtstr(lb, "simple nested formatting: %s(%S)");
 ...
 lb = logbuf_get(lc, 1, 1);
 if(lb != NULL) {
  logbuf_fmtstr(lb, "complex nested formatting: %s(%(10S%S%)%(20S, %S%)%(30S, %S%))");
  logbuf_string(lb, 1, "main");
 }
 sub1(lb, 10);
 sub2(lb, 20);
 ...
 { 
  ...
  lb = logbuf_get(lc, 1, 1);
  if(lb != NULL) {
   logbuf_fmtstr(lb, "binary tree: %(S%S%)");
